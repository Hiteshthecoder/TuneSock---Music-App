...
import 'package:tunesock/CustomWidgets/horizontal_albumlist.dart';
import 'package:tunesock/CustomWidgets/horizontal_albumlist_separated.dart';
import 'package:tunesock/CustomWidgets/image_card.dart';
import 'package:tunesock/CustomWidgets/like_button.dart';
import 'package:tunesock/CustomWidgets/on_hover.dart';
...
import 'package:tunesock/Helpers/format.dart';
import 'package:tunesock/Models/image_quality.dart';
import 'package:tunesock/Screens/Common/song_list.dart';
import 'package:tunesock/Screens/Library/liked.dart';
...
bool fetched = false;
List preferredLanguage = Hive.box('settings').get('preferredLanguage', defaultValue: ['Hindi']) as List;
...
class _SaavnHomePageState extends State<SaavnHomePage> with AutomaticKeepAliveClientMixin<SaavnHomePage> {
  ...
  List playlistNames = Hive.box('settings').get('playlistNames')?.toList() as List? ?? ['Favorite Songs'];
            ...
            itemBuilder: (context, idx) {
                ...
                return ValueListenableBuilder(
                  ...
                  builder: (BuildContext context, Box box, Widget? child) {
                    return (playlistNames.isEmpty ||
                            ...
                            (playlistNames.length == 1 && playlistNames.first == 'Favorite Songs' && likedCount() == 0))
                        ? const SizedBox()
                        : child!;
                  },
                );
              ...
              return (data[lists[idx]] == null ||
                      blacklistedHomeSections.contains(
                        data['modules'][lists[idx]]?['title']?.toString().toLowerCase(),
                      ))
                  ? const SizedBox()
                  : Column(
                      ...
                      children: [
                        Padding(
                          ...
                          child: Row(
                            children: [
                              Text(
                                data['modules'][lists[idx]]?['title']?.toString().unescape() ?? '',
                                style: TextStyle(
                                  color: Theme.of(context).colorScheme.secondary,
                                  fontSize: 18,
                                  fontWeight: FontWeight.bold,
                        ...
                        SizedBox(
                          ...
                          child: ListView.builder(
                            ...
                            scrollDirection: Axis.horizontal,
                            padding: const EdgeInsets.symmetric(horizontal: 10),
                            itemCount: data['modules'][lists[idx]]?['title']?.toString() == 'Radio Stations'
                                ? (data[lists[idx]] as List).length + likedRadio.length
                                : (data[lists[idx]] as List).length,
                            itemBuilder: (context, index) {
                              Map item;
                              if (data['modules'][lists[idx]]?['title']?.toString() == 'Radio Stations') {
                                        ...
                                        child: Stack(
                                          children: [
                                            ...
                                            AlertDialog(
                                              ...
                                              content: imageCard(
                                                borderRadius: item['type'] == 'radio_station' ? 1000.0 : 15.0,
                                ...
                                child: SizedBox(
                                  ...
                                  child: HoverBox(
                                    child: imageCard(
                                      ...
                                      borderRadius: item['type'] == 'radio_station' ? 1000.0 : 10.0,
                                      imageUrl: item['image'].toString(),
                                      imageQuality: ImageQuality.medium,
                                      placeholderImage: (item['type'] == 'playlist' || item['type'] == 'album')
                                          ? const AssetImage(
                                              'assets/album.png',
                                            )
                                          : item['type'] == 'artist'
