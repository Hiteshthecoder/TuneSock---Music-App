...
import 'package:tunesock/APIs/spotify_api.dart';
...
import 'package:tunesock/Helpers/spotify_country.dart';
import 'package:tunesock/Helpers/spotify_helper.dart';
// import 'package:tunesock/Helpers/countrycodes.dart';
import 'package:tunesock/Screens/Search/search.dart';
import 'package:tunesock/constants/countrycodes.dart';
...
Future<List> getChartDetails(String accessToken, String type) async {
  ...
  final String playlistId = type == 'Global' ? globalPlaylistId : localPlaylistId;
  final List data = [];
  final List tracks = await SpotifyApi().getAllTracksOfPlaylist(accessToken, playlistId);
...
Future<void> scrapData(String type, {bool signIn = false}) async {
  final bool spotifySigned = Hive.box('settings').get('spotifySigned', defaultValue: false) as bool;
      ...
      (uri) async {
        ...
        if (link.contains('code=')) {
          ...
          if (data.isNotEmpty) {
            ...
            Hive.box('settings').put('spotifyTokenExpireAt', currentTime + int.parse(data[2]));
          }
...
class _TopPageState extends State<TopPage> with AutomaticKeepAliveClientMixin<TopPage> {
  Future<void> getCachedData(String type) async {
    ...
    if (type == 'Global') {
      globalSongs = await Hive.box('cache').get('${type}_chart', defaultValue: []) as List;
    } else {
      localSongs = await Hive.box('cache').get('${type}_chart', defaultValue: []) as List;
    }
    setState(() {});
  }
  ...
  Widget build(BuildContext context) {
    ...
    return ValueListenableBuilder(
      ...
      builder: (BuildContext context, bool value, Widget? child) {
        ...
        return Column(
          children: [
            if (!(Hive.box('settings').get('spotifySigned', defaultValue: false) as bool))
              Expanded(
                child: Center(
                  child: TextButton(
                    onPressed: () {
