...
class SaavnAPI {
  ...
  Future<Response> getResponse(
    ...
    if (useProxy && settingsBox.get('useProxy', defaultValue: false) as bool) {
      final String proxyIP = settingsBox.get('proxyIp', defaultValue: '103.47.67.134').toString();
  ...
  Future<List<Map<String, dynamic>>> fetchSearchResults(
    ...
    if (res.statusCode == 200) {
      ...
      if (topQuery.isNotEmpty &&
          (topQuery[0]['type'] != 'playlist' || topQuery[0]['type'] == 'artist' || topQuery[0]['type'] == 'album')) {
  ...
  Future<Map<String, List>> fetchArtistSongs({
    ...
    if (res.statusCode == 200) {
      ...
      if (latestReleaseSearchedList.isNotEmpty) {
        data[getMain['modules']?['latest_release']?['title']?.toString() ?? 'Latest Releases'] =
      ...
      if (dedicatedSearchedList.isNotEmpty) {
        data[getMain['modules']?['dedicated_artist_playlist']?['title']?.toString() ?? 'Dedicated Playlists'] =
      ...
      if (featuredSearchedList.isNotEmpty) {
        data[getMain['modules']?['featured_artist_playlist']?['title']?.toString() ?? 'Featured Playlists'] =
      ...
      if (similarArtistsSearchedList.isNotEmpty) {
        data[getMain['modules']?['similarArtists']?['title']?.toString() ?? 'Similar Artists'] =
