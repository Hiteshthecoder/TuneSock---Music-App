...
import 'package:tunesock/Helpers/image_resolution_modifier.dart';
  ...
  static Future<Map> formatSingleAlbumSongResponse(Map response) async {
    try {
      ...
      return {
        ...
        'subtitle':
            '${response["primary_artists"].toString().trim()} - ${response["album"].toString().trim()}'.unescape(),
      ...
      return {
        ...
        'artist': response['music'] == null
            ? (response['more_info']?['music'] == null)
                ? (response['more_info']?['artistMap']?['primary_artists'] == null ||
                        (response['more_info']?['artistMap']?['primary_artists'] as List).isEmpty)
                    ? ''
