  ...
  if (savePath.trim() != '') {
    try {
      ...
      for (int i = 0; i < boxNames.length; i++) {
        ...
        try {
          await File(Hive.box(boxNames[i].toString()).path!).copy('$savePath/${boxNames[i]}.hive');
        } catch (e) {
          await [
      ...
      final String time = '${now.hour}${now.minute}_${now.day}${now.month}${now.year}';
      final zipFile = File('$savePath/${fileName ?? "BlackHole_Backup_$time"}.zip');
...
Future<void> restore(
  ...
  if (savePath != '') {
    ...
    try {
      ...
      ShowSnackBar().showSnackBar(context, AppLocalizations.of(context)!.importSuccess);
    } catch (e) {
