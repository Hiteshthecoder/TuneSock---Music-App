import 'package:tunesock/CustomWidgets/collage.dart';
import 'package:tunesock/CustomWidgets/custom_physics.dart';
import 'package:tunesock/CustomWidgets/data_search.dart';
...
import 'package:tunesock/CustomWidgets/playlist_head.dart';
import 'package:tunesock/CustomWidgets/song_tile_trailing_menu.dart';
import 'package:tunesock/Helpers/songs_count.dart' as songs_count;
import 'package:tunesock/Screens/Library/show_songs.dart';
...
class _LikedSongsState extends State<LikedSongs> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _tcontroller!.addListener(() {
      if ((_tcontroller!.previousIndex != 0 && _tcontroller!.index == 0) || (_tcontroller!.previousIndex == 0)) {
  ...
  void getLiked() {
    ...
    if (widget.fromPlaylist) {
      ...
      songs_count.addSongsCount(
        widget.playlistName,
        _songs.length,
        _songs.length >= 4 ? _songs.sublist(0, 4) : _songs.sublist(0, _songs.length),
      );
    }
    setArtistAlbum();
  }
  ...
  void sortSongs({required int sortVal, required int order}) {
    switch (sortVal) {
      case -1:
        final List order = Hive.box('settings').get('order_${widget.playlistName}', defaultValue: []) as List;
      ...
      case 0:
        _songs.sort(
          (a, b) => a['title'].toString().toUpperCase().compareTo(b['title'].toString().toUpperCase()),
        );
        break;
      case 1:
        _songs.sort(
          (a, b) => a['dateAdded'].toString().toUpperCase().compareTo(b['dateAdded'].toString().toUpperCase()),
        );
        break;
      case 2:
        _songs.sort(
          (a, b) => a['album'].toString().toUpperCase().compareTo(b['album'].toString().toUpperCase()),
        );
        break;
      case 3:
        _songs.sort(
          (a, b) => a['artist'].toString().toUpperCase().compareTo(b['artist'].toString().toUpperCase()),
        );
        break;
      case 4:
        _songs.sort(
          (a, b) => a['duration'].toString().toUpperCase().compareTo(b['duration'].toString().toUpperCase()),
        );
        break;
      default:
        _songs.sort(
          (b, a) => a['dateAdded'].toString().toUpperCase().compareTo(b['dateAdded'].toString().toUpperCase()),
        );
        break;
    }
          ...
          appBar: AppBar(
            title: Text(
              widget.showName == null
                  ? widget.playlistName[0].toUpperCase() + widget.playlistName.substring(1)
                  : widget.showName![0].toUpperCase() + widget.showName!.substring(1),
                      ...
                      ? Row(
                          children: [
                            MultiDownloadButton(
                              data: _songs
                                  .where(
                                    (element) => selectedItems.contains(element['id']),
                                  )
                                  .toList(),
                              playlistName: widget.showName == null
                                  ? widget.playlistName[0].toUpperCase() + widget.playlistName.substring(1)
                  ...
                  itemBuilder: (context, index) {
                    return ValueListenableBuilder(
                      ...
                      builder: (context, value, child) {
                        final bool selected = selectedItems.contains(widget.songs[index]['id']);
                        return ListTile(
                          leading: imageCard(
                          ...
                          onTap: () {
                            if (selectMode.value) {
                              ...
                              if (selected) {
                                ...
                                selectedItems.add(widget.songs[index]['id'].toString());
                                selectMode.value = true;
                              }
                              setState(() {});
                            } else {
